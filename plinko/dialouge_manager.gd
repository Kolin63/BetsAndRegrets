...
func do_dia(index):
	...
	# one zero
		...
		load("res://vox/Game over buddy.wav"), 
		index,
		...
		load("res://vox/Back to start.wav"),
		index,
		null)
		set_man("talking")
	...
	# two zero
		...
		load("res://vox/Gotta be more careful .wav"), 
		index,
		null)
		set_man("miserable")
	...
	# three zero
		...
		load("res://vox/Only have 1 shot left.wav"), 
		index,
		load("res://music/Dnd is satanic.mp3"))
		set_man("devil_talk")
	elif (index == 5.1):
		...
		load("res://vox/No, gotta be more careful .wav"), 
		index,
		null)
		set_man("miserable")
	...
	# win
		...
		load("res://vox/Grr you've won.wav"), 
		index,
		null)
		set_man("devil_talk")
	elif (index == 6.1):
		...
		load("res://vox/Yes i am free.wav"), 
		index,
		...
		load("res://vox/Narrator win scenario .wav"), 
		index,
		)
		set_man("hide")
	elif (index == 6.1 + 0.1 + 0.1):
		get_tree().change_scene_to_file("res://transition_scenes/win.tscn")
	...
	# lose
		...
		load("res://vox/Narrator lose scenario .wav"), 
		index,
	...
	# fades out as he becomes the devil's servant, game over screen
		...
		get_tree().change_scene_to_file("res://transition_scenes/lose.tscn")
	
	# when he wins a day, has max souls, earnings are above 2x exclusive
	elif (index == 8):
		Bubble.say("I guess it's something",
		load("res://vox/I guess it's something .wav"),
		index)
		set_man("talking")
	...
	# after he loses 3 times in a row, he has max souls
		...
		load("res://vox/This game is rigged .wav"),
		index)
		set_man("sad")
	...
	# after he wins and no other conditions are met
		...
		if (rand == 0):
			Bubble.say("I won!",
			load("res://vox/I won.wav"),
			index)
			set_volume(-10)
		elif (rand == 1):
			Bubble.say("Finally!",
			load("res://vox/Finally.wav"),
			index)
		set_man("happy")
	...
	# After he loses a day after losing 2 souls
		...
		if (rand == 0):
			...
			load("res://vox/Miss family .wav"),
			index)
		elif (rand == 1):
			Bubble.say("It's okay, I can get it back... ",
			load("res://vox/I can get it back....wav"),
			index)
		set_man("sad")
	...
	# After he wins a day after losing 2 souls
		...
		if (rand == 0):
			...
			load("res://vox/Oh yes hope.wav"),
			index)
		elif (rand == 1):
			Bubble.say("D-do I have a chance? ",
			load("res://vox/D-do I have a chance.wav"),
			index)
		set_man("happy")
	...
	# when he loses a day after he lost 3 souls
		...
		if (rand == 0):
			...
			load("res://vox/Im sorry liar .wav"),
			index)
		elif (rand == 1):
			Bubble.say("Cheating at life landed me here... ",
			load("res://vox/Cheating .wav"),
			index)
		set_man("sad")
	...
	# when he loses and goes below 9M
		...
		load("res://vox/I was so close! .wav"),
		index)
		set_man("sad")
	...
	# when he wins a day after losing 3 souls
		...
		load("res://vox/I have a chance .wav"),
		index)
		set_man("happy")
